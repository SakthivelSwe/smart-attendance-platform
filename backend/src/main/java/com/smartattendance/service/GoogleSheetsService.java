...
public class GoogleSheetsService {
    ...
    /**
     * Updates multiple attendance records in a single batch API call for a specific
     * spreadsheet.
     */
    public String updateAttendanceBatch(String spreadsheetId, String expectedMonth, List<Attendance> records) {
        if (spreadsheetId == null || spreadsheetId.isBlank()) {
            logger.info("No Spreadsheet ID configured, skipping batch sync.");
        ...
        try {
            ...
            for (com.google.api.services.sheets.v4.model.Sheet sheet : sheetList) {
                String title = sheet.getProperties().getTitle().trim();
                // Check if title matches expected patterns
                if (title.equalsIgnoreCase(shortMonth + year2Digit) ||
                        title.equalsIgnoreCase(shortMonth + " " + year2Digit) ||
                        title.equalsIgnoreCase(fullMonth + " " + year4Digit) ||
                        (title.toLowerCase().startsWith(shortMonth.toLowerCase()) && title.contains(year2Digit))) {
            ...
            // 4. Find the "Employee Name" header cell
            int headerRowIndex = -1;
            int employeeNameColIndex = -1;
            ...
            // Cache for fast repetitive lookups
            java.util.Map<LocalDate, Integer> dateColumnIndexes = new java.util.HashMap<>();
            java.util.Map<String, Integer> employeeRowIndexes = new java.util.HashMap<>();

            // Build employee row lookup
            for (int r = headerRowIndex + 1; r < values.size(); r++) {
                List<Object> row = values.get(r);
                if (row.size() > employeeNameColIndex) {
                    Object cell = row.get(employeeNameColIndex);
                    if (cell instanceof String rowEmpName) {
                        String normalizedSheetName = rowEmpName.replaceAll("[\\s\\u00A0]+", "").trim().toLowerCase();
                        if (!normalizedSheetName.isEmpty()) {
                            employeeRowIndexes.put(normalizedSheetName, r);
                        }
                    }
                }
            }

            List<ValueRange> updateData = new java.util.ArrayList<>();
            ...
            for (Attendance record : records) {
                ...
                // 5. Find the Date column index
                Integer dateColumnIndex = dateColumnIndexes.get(recordDate);
                if (dateColumnIndex == null) {
                    Double targetSerialDate = getExcelSerialDate(recordDate);
                    for (int c = employeeNameColIndex + 1; c < headerRow.size(); c++) {
                        Object cell = headerRow.get(c);
                        if (cell == null)
                            continue;
                        ...
                        if (cell instanceof Number num) {
                            ...
                            str = str.trim();
                            if (str.equalsIgnoreCase(recordDate.format(fmt1)) ||
                                    str.equalsIgnoreCase(recordDate.format(fmt2)) ||
                                    str.equalsIgnoreCase(recordDate.format(fmt3)) ||
                                    str.equalsIgnoreCase(recordDate.format(fmt4)) ||
                                    str.equalsIgnoreCase(recordDate.format(fmt5)) ||
                                    str.equals(String.format("%02d", recordDate.getDayOfMonth())) ||
                                    str.equals(String.valueOf(recordDate.getDayOfMonth()))) {
                                match = true;
                            }
                        }
                        ...
                        if (match) {
                            dateColumnIndex = c;
                            dateColumnIndexes.put(recordDate, c);
                            break;
                        }
                    }
                }
                ...
                if (dateColumnIndex == null) {
                    logger.warn("Could not find column for date '{}' in sheet '{}'", recordDate, sheetName);
                    continue; // Skip this record
                }

                // 6. Find the Employee Row index
                String normalizedDbName = employee.getName().replaceAll("[\\s\\u00A0]+", "").trim().toLowerCase();
                Integer employeeRowIndex = employeeRowIndexes.get(normalizedDbName);

                if (employeeRowIndex == null) {
                    logger.warn("Could not find row for employee '{}' in sheet '{}'", employee.getName(), sheetName);
                    continue; // Skip this record
                }
                ...
                updateData.add(new ValueRange()
                        .setRange(targetRange)
                        .setValues(Collections.singletonList(Collections.singletonList(valueToWrite))));
            }

            if (updateData.isEmpty()) {
                logger.warn("No valid update cells mapped to sheet '{}'", sheetName);
                return "No valid updates";
            }

            // 7. Execute batch update
            com.google.api.services.sheets.v4.model.BatchUpdateValuesRequest batchRequest = new com.google.api.services.sheets.v4.model.BatchUpdateValuesRequest()
                    .setValueInputOption("USER_ENTERED")
                    .setData(updateData);
            ...
            while (attempt < maxRetries && !success) {
                try {
                    sheetsService.spreadsheets().values()
                            ...
                            .execute();
                    success = true;
                    logger.info("Successfully synced {} attendance records (batch) to sheet {}", updateData.size(),
                            sheetName);
                    return "Batch Success";
                } catch (com.google.api.client.googleapis.json.GoogleJsonResponseException e) {
                    if (e.getStatusCode() == 429) {
                        attempt++;
                        logger.warn("Rate limit exceeded for batch update (429). Retrying attempt {}/{}", attempt,
                                maxRetries);
                        try {
                            Thread.sleep((long) Math.pow(2, attempt) * 1000); // Exponential backoff: 2s, 4s, 8s
                        } catch (InterruptedException ie) {
            ...
            if (!success) {
                logger.error("Failed to batch update Google Sheet after {} attempts", maxRetries);
                return "Failed attempts";
            }
            ...
            try {
                details = e.getDetails().toPrettyString();
                logger.error("Failed to batch update Google Sheet, API Error: \n" + details, e);
            } catch (Exception ex) {
                logger.error("Failed to batch update Google Sheet, API Error: " + e.getMessage(), e);
            }
            throw new RuntimeException("Google Sheets API reported: " + e.getMessage(), e);
        } catch (Exception e) {
            logger.error("Failed to batch update Google Sheet: {}", e.getMessage(), e);
            throw new RuntimeException("Failed to batch update Google Sheet", e);
        }
        return "Unknown error";
    }
